...
RolloutBuffer::RolloutBuffer(const size_t num_envs, const size_t reserve)
...
RolloutSample RolloutBuffer::get(size_t index)
{
    size_t i = 0;
...
torch::optional<size_t> RolloutBuffer::size() const
{
    size_t size = 0;
    for (size_t i = 0; i < data.size(); ++i)
    {
    ...
    const float gamma, const float lambda_gae)
{
    ...
    for (size_t env = 0; env < data.size(); ++env)
    {
